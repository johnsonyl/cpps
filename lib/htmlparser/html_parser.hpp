...
public:
    ...
    std::vector<shared_ptr<HtmlElement> > GetAllElements() {
        std::vector<shared_ptr<HtmlElement> > result;
    ...
    void SelectElementByRule(const std::string& rule, std::vector<shared_ptr<HtmlElement> >& result) {
        ...
        if (rule.size() >= 2 && rule.at(1) == '/') {
            ...
            if (next.empty()) {
                ...
                for (size_t i = 0; i < temp.size(); i++) {
                    temp[i]->SelectElementByRule(next, result);;
                }
            }
        }
        else {
            ...
            if (matched) {
                if (next.empty())
                    ...
                    for (ChildIterator it = ChildBegin(); it != ChildEnd(); it++) {
                        (*it)->SelectElementByRule(next, result);
                    }
                }
            }
        };
	...
	void SelectElement(const std::string& rule, std::vector<shared_ptr<HtmlElement> > &result) {
		HtmlElement::ChildIterator it = ChildBegin();
		for (; it != ChildEnd(); it++) {
			(*it)->SelectElementByRule(rule, result);
		}
	}
...
public:
    ...
    shared_ptr<HtmlElement> GetRoot()
    {
        return root_;
    }
    std::string html() {
